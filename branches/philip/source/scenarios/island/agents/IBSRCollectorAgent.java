...
/**
 * Die Klasse stellt die Superklasse für alle Sammelagenten der IBSR-Strategie
 * da, sie muss in den Subklassen noch mit den passenden Classifiern versehen 
 * werden.
 * 
 * @author Daniel Friedrich
 */
public class IBSRCollectorAgent extends IBCollectorAgent {
	 ...
	 * @param handler
	 */
	public IBSRCollectorAgent(String name, AbstractScenarioHandler handler) {
		super(name, handler);
		m_isALearningAgent = true;
	}
	 ...
	 * @param name
	 */
	public IBSRCollectorAgent(Id id, String name) {
		super(id, name);
		m_isALearningAgent = true;
	}
		...
		try {
			...
			if(!this.getScenarioHandler().isPlaceExplored(
				...
				if(this.shouldIExploreThePlace(attributes)) {
					...
					System.out.println("#### NICHT GRABEN ####");
				}			
			}	
