/*
 * Dateiname      : NektarKooperativerAgent.java
 * Erzeugt        : 20. Julii 2005
 ...
 * Der Agent ist kooperativ und verwendet Informationen über den Nektarinhalt einer Blume 
 * für die Steuerung der Kommunikation und Kooperation.
 */
public class NektarKooperativerAgent
    ...
    /**
     * Ein Kooperativer Agent, der die Information über die Nektarmenge einer Blumenwiese
     * ausnutzt.
     * @param bName Name des Agenten
     * @param bHandler Handler des Agenten
     */
    public NektarKooperativerAgent(String bName, AbstractScenarioHandler bHandler) {
     ...
     * ausnutzt.
     */
    public NektarKooperativerAgent() {
        super();
        volksID = 1;
        //  zufallsGenerator.setSeed(samen);
    }
     ...
     * @param bName Name des Agenten
     */
    public NektarKooperativerAgent(Id bId,String bName) {
        super(bId, bName);
        name = bName;
        volksID = 1;
    }
    ...
    /**
     * liefert die Volkszugehörigkeit des Agenten.
     * @return Nummer des Bienenvolkes, zu der die Biene gehört.
     */
            ...
            case DesireIntentionPlan.G_FINDEDIEBLUME:{
                if (myPosition.equals(altesZiel)){
                    if (bekannteBlumen.containsKey(altesZiel)){
                        modus.setDesire(DesireIntentionPlan.G_BLUMENPROBEENTNEHMEN);
                        modus.setDesireZiel(altesZiel);
                        evaluateGoal(modus);
                    }else {
                        Koordinate neueZielKoord=sucheBlumeInNachbarfelder();
                        if (neueZielKoord==null) {
                            modus.setDesire(DesireIntentionPlan.G_FINDEEINEBLUME);
                        } else {
                            modus.setDesire(DesireIntentionPlan.G_BLUMENPROBEENTNEHMEN);
                            modus.setDesireZiel(neueZielKoord);
                    ...
                    && (this.rundeNrKooperation<=this.maxAnzahlKooperationsRunden) && !this.bieneIstInDerLuft){
                        ...
                        if (this.neueMitteilungErhalten){
                            ...
                            if (vorsAnzTours<beta) {
                                ...
                                int agentenAnz=bearbBlumeAgenten.size();
                                double zufall=Math.random();
                                if (zufall<=0.7 || agentenAnz<=3) {
    ...
    private void addIgnoredBienen(){
        if (this.positionFeld instanceof EinfacheBlume){
            if (selbst.gibGeladeneNektarmenge()==0 && this.bieneIstInDerLuft) {
                this.ignorierteBienenId.clear();
